...
bool IsReluSemanticMaximum(const RemapperContext& ctx,
                           const utils::MutableNodeView& node_view) {
  if (!IsMaximum(*node_view.node())) return false;

  const std::vector<OpInfo::TensorProperties>& input_props =
      ctx.graph_properties.GetInputProperties(node_view.node()->name());
  if (input_props.size() < 2) return false;

  for (const auto& input_prop : input_props) {
    if (input_prop.shape().dim_size() == 0 &&
        IsZeroTensor(input_prop.value(), input_prop.dtype())) {
      return true;
      break;
    }
  }
...
bool FindContractionWithBiasAddAndActivation(
  ...
  if (!IsSupportedActivation(*node_def, cluster) &&
      !(IsReluSemanticMaximum(ctx, *node_view)))
    return false;
...
Status AddFusedContractionNode(
  ...
  SetFusedOpAttributes(
      &fused_op,
      {"BiasAdd",
       IsReluSemanticMaximum(*ctx, *ctx->graph_view.GetNode(matched.activation))
           ? "Relu"
           : activation.op()});
...
Status GetTensorFromConstantOp(const NodeDef* node_def, Tensor* dst) {
  ...
  DCHECK(dst != nullptr) << "Input tensor must not be null";
...
bool FindDilatedConv(RemapperContext* ctx, int node_index,
  ...
  if ((stb_block_shape_tensor.NumElements() != 2) ||
      (stb_block_shape_tensor.shape() != bts_block_shape_tensor.shape())) {
    return false;
  };

  DataType dtype = stb_block_shape_tensor.dtype();
  if ((dtype != DT_INT32 && dtype != DT_INT64) ||
      (dtype != bts_block_shape_tensor.dtype())) {
    return false;
  }
  std::vector<int64> stb_block_data =
      (dtype == DT_INT32)
          ? GetTensorValues<int32, int64>(stb_block_shape_tensor)
          : GetTensorValues<int64, int64>(stb_block_shape_tensor);
  std::vector<int64> bts_block_data =
      (dtype == DT_INT32)
          ? GetTensorValues<int32, int64>(bts_block_shape_tensor)
          : GetTensorValues<int64, int64>(bts_block_shape_tensor);

  // Check block values in SpaceToBatchND and BatchToSpaceND, the upper
  // bound is used to exclude handling of invalid dilation rate case.
  for (int i = 0; i < stb_block_data.size(); i++) {
    if (stb_block_data[i] != bts_block_data[i] || stb_block_data[i] < 1 ||
        stb_block_data[i] > 1000) {
      return false;
    }
  }
  ...
  Tensor crops_tensor;
  TF_CHECK_OK(GetTensorFromConstantOp(stb_paddings_node, &paddings_tensor));
  TF_CHECK_OK(GetTensorFromConstantOp(bts_crops_node, &crops_tensor));
  dtype = paddings_tensor.dtype();
  if ((dtype != DT_INT32 && dtype != DT_INT64) ||
      (dtype != crops_tensor.dtype()))
    return false;
  if ((paddings_tensor.shape().dims() != 2) ||
      (paddings_tensor.shape().dim_size(0) != 2) ||
      (paddings_tensor.shape().dim_size(1) != 2) ||
      (paddings_tensor.shape() != crops_tensor.shape())) {
    return false;
  };

  std::vector<int32> conv_strides;
  if (!TryGetNodeAttr(*conv_node, "strides", &conv_strides)) return false;
  if (std::any_of(conv_strides.begin(), conv_strides.end(),
                  [](int32 stride) { return stride != 1; })) {
    return false;
  }
...
std::vector<int32> ComputePaddings(const Tensor& paddings, const Tensor& crops,
  ...
  auto paddings_data = paddings.flat<T>();
  auto crops_data = crops.flat<T>();
...
Status AddDilatedConvNode(RemapperContext* ctx,
  ...
  if ((dtype != DT_INT32 && dtype != DT_INT64) || dtype != dtype2) {
    TF_CHECK_OK(errors::InvalidArgument(
        "Unexpected data type with paddings: ", dtype, ", crops: ", dtype2));
  }

  std::vector<int32> real_paddings;
  bool is_valid = true;
  if (dtype == DT_INT32) {
    real_paddings = ComputePaddings<DT_INT32>(paddings, crops, is_valid);
  } else {
    real_paddings = ComputePaddings<DT_INT64>(paddings, crops, is_valid);
  }

  // Follow the MLIR implementation with using "SAME" attribute
...
bool RequiresInferredShapes(const RemapperContext& ctx, int node_index,
  ...
  const auto is_add_matmul_candidate = [&]() -> bool {
    if (!IsAdd(*node_def) && !IsBiasAdd(*node_def)) return false;
    if (node_view->NumRegularFanins() < 2) return false;
    const auto& add_fanin_0 = node_view->GetRegularFanin(0);
  ...
  const auto is_maximum_add_matmul_candidate = [&]() -> bool {
  ...
  if (IsMKLEnabled())
    return is_batch_norm_candidate() || is_batch_norm_fusion_candidate() ||
           ...
           IsTranspose(*node_def) || is_maximum_add_matmul_candidate() ||
  ...
  return is_act_biasadd_conv_candidate() || is_batch_norm_candidate() ||
         ...
         is_maximum_add_matmul_candidate() || is_add_matmul_candidate();
}
