...
std::optional<MatchedFp8Param> MatchFp8Param(HloInstruction *instr) {
  ...
  if (subgraph.size() == 1) {
    ...
    return MatchedFp8Param{subgraph[0].first, nullptr, false, {}};
  }
  ...
  if (subgraph.size() > 2 &&
      Match(subgraph[2].first,
            m::MultiplyAnyOrder(m::Convert(m::Op(&fp8_input)),
                                m::Broadcast(m::Op(&scale))))) {
    mult_scale = true;
    num_dequant_ops = 2;
  } else if (subgraph.size() > 2 &&
             Match(subgraph[2].first, m::Divide(m::Convert(m::Op(&fp8_input)),
                                                m::Broadcast(m::Op(&scale))))) {
    mult_scale = false;
    num_dequant_ops = 2;
  } else if (subgraph.size() > 1 &&
             Match(subgraph[1].first, m::Convert(m::Op(&fp8_input)))) {
  ...
  InstrPath commutative_ops = {subgraph.begin() + start, subgraph.end()};
  return MatchedFp8Param{fp8_input, scale, mult_scale, commutative_ops};
}
