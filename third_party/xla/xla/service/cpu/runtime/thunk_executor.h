...
class ThunkExecutor {
 public:
  ...
  // produced by the executor. It can be simple inline executor that runs tasks
  // on the same thread, or a runner backed by a thread pool.
  ...
  // Executes the thunk sequence using prepared dataflow graph. Executor uses
  // runner to execute ready tasks concurrently. If runner is not provided,
